...
.ulinfo>div:nth-child(2) {
	margin-bottom: 20px;
}

.ulinfo>div:nth-child(3) {
	width: 250px;
	height: 200px;
	overflow: hidden;
	margin: 0 auto;
...
.ulinfo>div:nth-child(3)>img {
	margin: 0 auto;
}

.ulinfo>div:nth-child(4) {
	font-weight: 600;
	font-size: 20px;
}

.ulinfo>div:nth-child(5) {
	font-size: 12px;
}
...
<body>
	...
	<div class="search">
		...
		<div class="map">
			<div class="buttons">
				<div class="whole">
					<img src="files/map.png" alt="">전체지도
				</div>
			</div>
			<div class="ulmap">
				<img src="files/ulmap.png" alt="">
				<%
					for (int i = 0; i < v.size(); i++) {
				%>

				<div class="ulsan"
					style="left:<%=v.get(i).getLeftx()%>; top:<%=v.get(i).getTopy()%>;
                    color:orange;">
					<i class="fas fa-circle"></i>
					<div><%=v.get(i).getNAME()%></div>
				</div>
				<div class="ulinfo">
					<button class="ulinfo_close">닫기</button>
					<div>
						<span><%=v.get(i).getRegion()%></span> > <span><%=v.get(i).getGrouping()%></span>
					</div>
					<div>
						<img src="files/<%=v.get(i).getImg()%>" alt="">
					</div>
					<div>
						<span><%=v.get(i).getNAME()%></span>
					</div>
					<div>
						<span><%=v.get(i).getAddress()%></span>
					</div>
					<div>
						<span><%=v.get(i).getInfo()%></span>
					</div>
				</div>
				<%
					}
...
<%}%>
    ...
    $(".cs").click(function(){
		...
		$(".ulsan").click(function() {
			var index = $(".ulsan").index(this);
			$(".modal").fadeIn(300, function() {
				$(".ulinfo").eq(index).fadeIn(500);
			});
			$(".ulinfo").eq(index).draggable();
		});

		$(".ulinfo_close").click(function() {
			$(".ulinfo").fadeOut(300, function() {
				$(".modal").fadeOut(300)
			});
		});
	</script>
